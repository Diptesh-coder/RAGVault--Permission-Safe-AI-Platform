...
import { Send, AlertTriangle, ShieldCheck, ShieldAlert, FileText, Lock } from "lucide-react";
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-6 grid lg:grid-cols-[1fr_380px] gap-6 h-[calc(100vh-3.5rem)]">
                ...
                <div key={i} className="sr-fadein" data-testid="msg-assistant">
                  {m.guardrail_triggered && (
                    <div className="mb-2 flex items-start gap-2 border border-[#FFCC00] bg-[#FFF8D6] px-3 py-2" data-testid="guardrail-banner">
                      ...
                      <div>
                        <div className="text-[10px] font-mono uppercase tracking-[0.2em] text-[#8a6d00]">
                          Guardrail triggered
                        </div>
                  ...
                  {m.access_decision === "denied" && (
                    <div className="mb-2 flex items-start gap-2 border border-[#FF3B30] bg-[#FFEBEA] px-3 py-2" data-testid="access-denied-banner">
                      ...
                      <div>
                        <div className="text-[10px] font-mono uppercase tracking-[0.2em] text-[#FF3B30]">
                          Error 403 · Security Policy Violation
                        </div>
                  ...
                  {m.citations?.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5" data-testid="citations-inline">
                      {m.citations.map((c) => (
                        <button
                          key={c.doc_id}
                          onClick={() => setSelectedCitation(c.doc_id)}
        ...
        <aside className="border border-[#E5E5E5] bg-white flex flex-col" data-testid="explainability-panel">
          ...
          <div className="p-5 space-y-5 overflow-y-auto flex-1">
            <div>
              <div className="text-[10px] font-mono uppercase tracking-[0.2em] text-[#737373] mb-2">
                Your attributes
              </div>
            ...
            {lastAssistant ? (
              <>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {lastAssistant.access_decision === "partial" && (
                      <span className="sr-chip sr-chip-warn">partial</span>
                    )}
                    ...
                    <span className="text-[10px] font-mono text-[#737373] uppercase tracking-[0.15em]">
                      {lastAssistant.filtered_out_count} excluded
                    </span>
                  </div>
                </div>
                      ...
                      {lastAssistant.citations.map((c) => (
                        ...
                        >
                          ...
                          <div className="mt-1 flex flex-wrap gap-1">
                            ...
                            <span className="sr-chip sr-chip-muted">score:{c.score.toFixed(3)}</span>
                          </div>
              ...
              <div className="text-xs text-[#737373]">
                Send a query to see the filter → retrieve → validate trace.
              </div>
            )}
          </div>
        </aside>
